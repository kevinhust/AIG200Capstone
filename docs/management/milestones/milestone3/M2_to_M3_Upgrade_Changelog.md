...
## 14. v7.0 Proactive Nudging (2026-03-11)

### 14.1 Time Window Detection

**New Files:**
- `src/scheduler/__init__.py`
- `src/scheduler/time_window_detector.py`

**Purpose:** Transform bot from "passive responder" to "proactive caregiver"
...
def detect_time_windows(
    ...
    """
    Extract recurring time patterns from workout history.

    Grouping: (exercise_name, day_of_week, hour_bucket)
    Confidence: freq_confidence (max 0.7) + recency_bonus (max 0.3)
    """
```
...
|------|----------|---------|
| `proactive_workout_nudge` | Hourly 7:00-22:00 | Check time windows, send reminders |
| `refresh_time_windows` | Daily 6:00 AM | Refresh pattern detections |

**Implementation:**
```python
@tasks.loop(time=[time(h, 0, tzinfo=pu.LOCAL_TZ) for h in range(7, 23)])
async def proactive_workout_nudge(self):
    """Proactive workout reminders based on detected time patterns."""
    # 1. Get all users with detected patterns
    # 2. Check current time window matches
    # 3. Verify not already nudged today
    # 4. Send personalized nudge with budget context
```

### 14.3 Proactive Nudge View

**File:** `src/discord_bot/views.py`

**Class:** `ProactiveNudgeView`

| Button | Style | Action |
|--------|-------|--------|
| Too Tired | Red | Record as skipped (fatigue) |
| Maybe Later | Gray | Neutral response |
| Feeling Good! | Green | Positive feedback |
| Start Now | Blurple | Initiate workout logging |

### 14.4 Nudge Status Tracking

**File:** `src/discord_bot/profile_utils.py`

```python
# Memory caches
_user_time_windows: Dict[str, List[Dict[str, Any]]] = {}
_user_nudge_status: Dict[str, str] = {}  # "2026-03-11" format
```

---

## 15. v7.1 MET Science Engine (2026-03-12)

### 15.1 MET-Based Calorie Calculation

**New File:** `src/data_rag/met_mapping.py`

**Formula:** `Calories = MET × Weight(kg) × Duration(hours)`

**Reference:** Compendium of Physical Activities (Ainsworth et al., 2011)

**MET Reference Database:**
```python
MET_REFERENCE = {
    # Low Intensity (MET < 3.0)
    "yoga": (2.5, "low"),
    "stretching": (2.3, "low"),
    "walking slow": (2.0, "low"),

    # Moderate Intensity (MET 3.0-6.0)
    "walking brisk": (4.3, "moderate"),
    "cycling moderate": (5.8, "moderate"),
    "swimming moderate": (5.0, "moderate"),

    # High Intensity (MET > 6.0)
    "running": (9.8, "high"),
    "hiit": (10.0, "high"),
    "jump rope": (11.0, "high"),
    # ... 80+ exercise mappings
}
```

### 15.2 Intensity Classification

| Level | MET Range | Examples |
|-------|-----------|----------|
| Low | < 3.0 | Yoga, Stretching, Light Walking |
| Moderate | 3.0 - 6.0 | Brisk Walking, Cycling, Swimming |
| High | > 6.0 | Running, HIIT, Jump Rope |

### 15.3 Exercise Cache Enrichment

**New File:** `scripts/enrich_exercise_cache.py`

**Statistics (867 Wger exercises):**

| Metric | Count | Percentage |
|--------|-------|------------|
| Total Exercises | 867 | 100% |
| With MET Values | 867 | 100% |
...
### 15.4 Three-Tier Decision Priority

**File:** `src/agents/fitness/fitness_agent.py`

| Priority | Tier | Rule |
|----------|------|------|
| 1 | Safety First | Health conditions avoid contraindicated movements |
| 2 | Budget Matching | Remaining > 300 kcal → High; 100-300 → Moderate; < 100 → Low |
| 3 | Scenario Intelligence | Home → Bodyweight; Gym → Full equipment |

### 15.5 Visual Exercise Guide

**Enhanced:** `src/discord_bot/embed_builder.py`

**New Parameters for `build_proactive_nudge_embed()`:**
- `image_url`: Exercise demonstration image from Wger
- `met_value`: Scientific MET value
- `intensity`: Intensity level with badge (🟢/🟡/🔴)

### 15.6 Test Command

**New Command:** `/test_nudge [exercise]`

**Purpose:** Test proactive nudge visualization with exercise images

**Usage:**
```
/test_nudge              # Random exercise with image
/test_nudge Push         # Push-up related exercise
/test_nudge Bench Press  # Bench press variations
```

---

## 16. Multi-Agent Swarm Architecture

### 16.1 Overview

Health Butler uses a **Custom Swarm Implementation** (not LangGraph or LangChain). The swarm architecture is built from scratch with:

- **Zero external dependencies** for orchestration
- **Lightweight async execution** via `asyncio`
- **Simple handoff protocol** using string signals
- **JSON-based context sharing** via Health Memo

**Why Custom Swarm over LangGraph?**

| Factor | Custom Swarm | LangGraph |
|--------|--------------|-----------|
| Dependencies | Zero | +langchain, +langgraph |
| Latency | < 100ms overhead | +500ms graph compilation |
| Flexibility | Full control | Constrained by framework |
| Learning Curve | Low | High |
| Debug | Direct | Graph visualization needed |

### 16.2 Core Files

| File | Purpose |
|------|---------|
| `src/swarm.py` | HealthSwarm orchestrator class |
| `src/agents/router_agent.py` | Task analysis and delegation |
| `src/agents/base_agent.py` | Abstract base for all agents |
| `src/coordinator/coordinator_agent.py` | Multilingual intent routing |

### 16.3 Architecture

Health Butler uses a **Swarm Architecture** where specialized agents collaborate through a central orchestrator (RouterAgent). This approach enables:

- **Modularity**: Each agent is independently developed and tested
- **Scalability**: New agents can be added without modifying existing code
- **Context Handoff**: Agents share context via Health Memo Protocol
- **Graceful Degradation**: System continues if individual agents fail

### 16.2 Swarm Components

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        Health Swarm Architecture                         │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│    ┌─────────────┐                                                      │
│    │   Discord   │  User Input + Image                                  │
│    │    Bot      │──────────────────────────────────┐                   │
│    └─────────────┘                                  │                   │
│                                                     ▼                   │
│    ┌─────────────────────────────────────────────────────────────────┐  │
│    │                      HealthSwarm (Orchestrator)                 │  │
│    │                         (src/swarm.py)                          │  │
│    │  ┌─────────────────────────────────────────────────────────┐    │  │
│    │  │  • Receives user input                                   │    │  │
│    │  │  • Checks for explicit handoff signals                   │    │  │
│    │  │  • Delegates to RouterAgent for analysis                 │    │  │
...
│     ▼               ▼                 ▼                 ▼              │
│ ┌─────────┐   ┌───────────┐   ┌───────────┐   ┌─────────────┐        │
│ │ Fitness │   │ Nutrition │   │ Engagement│   │  Analytics  │        │
│ │  Agent  │   │   Agent   │   │   Agent   │   │    Agent    │        │
│ └────┬────┘   └─────┬─────┘   └───────────┘   └─────────────┘        │
│      │              │                                                  │
│      │    ┌─────────┴─────────┐                                       │
│      │    │   Health Memo     │  Cross-Agent Context                  │
│      └───►│    Protocol       │◄───────────────────────────────────┘  │
│           │ (TypedDict)       │                                        │
│           └───────────────────┘                                        │
│                                                                        │
...
### 16.3 Agent Specializations

| Agent | File | Responsibility |
|-------|------|----------------|
| **RouterAgent** | `src/agents/router_agent.py` | Task analysis, delegation, synthesis |
| **FitnessAgent** | `src/agents/fitness/fitness_agent.py` | Workout recommendations, MET calculations |
| **NutritionAgent** | `src/agents/nutrition/nutrition_agent.py` | Meal analysis, macro calculations |
| **EngagementAgent** | `src/agents/engagement/engagement_agent.py` | User motivation, check-ins |
| **AnalyticsAgent** | `src/agents/analytics/analytics_agent.py` | Trends, forecasts, anomalies |

### 16.4 Handoff Protocol
...
def handoff_to_nutrition(kcal_burned: Optional[float] = None) -> str:
    """Signal to handoff to Nutrition Agent after workout."""
    return f"transfer_to_nutrition:{kcal_burned or 0}"

def handoff_to_fitness() -> str:
    """Signal to handoff to Fitness Agent."""
    return "transfer_to_fitness"
```

**Handoff Flow Example:**
```
User: "I just ran 5km, what should I eat?"

1. RouterAgent → Delegates to FitnessAgent
2. FitnessAgent → Calculates burned calories (MET × weight × duration)
3. FitnessAgent → Returns kcal_burned = 350
4. HealthSwarm → Detects nutrition context needed
5. Handoff to NutritionAgent with kcal_burned context
6. NutritionAgent → Suggests recovery meal based on deficit
```

### 16.5 Health Memo Protocol

**Purpose:** Pass structured context between agents
...
**Usage in FitnessAgent:**
```python
# Extract visual warnings from NutritionAgent's analysis
warnings = memo.get("visual_warnings", [])
if "fried" in warnings:
    # Reduce intensity recommendations
    recommendations = [ex for ex in recommendations if ex["intensity"] != "high"]
```

### 16.6 Delegation Flow

```python
# RouterAgent.analyze_and_delegate() output format
delegations = [
    {"agent": "fitness", "task": "Suggest leg workout for intermediate"},
    {"agent": "nutrition", "task": "Recommend post-workout meal"}
]

# HealthSwarm.execute_async() processes each delegation
for delegation in delegations:
    if delegation["agent"] == "fitness":
        agent = FitnessAgent()
        result = await agent.execute_async(task, context)
    elif delegation["agent"] == "nutrition":
        agent = NutritionAgent()
        result = await agent.execute_async(task, context)

# Final synthesis
final_response = router.synthesize_results(delegations, results)
```

### 16.7 Fallback Mechanisms

| Scenario | Fallback |
|----------|----------|
| Router parsing fails | `_simple_delegate()` keyword matching |
| Agent unavailable | Skip delegation, continue with others |
| Image processing fails | Text-only nutrition analysis |
| Supabase offline | In-memory cache fallback |

---

## 17. Architecture Diagram (v7.1)

```
┌─────────────────────────────────────────────────────────────────────┐
│                      Health Butler v7.1 Architecture                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────────────────┐ │
...
│  │  │ • Auto-enriched with MET + Intensity + Equipment        │ │  │
│  │  └─────────────────────────────────────────────────────────┘ │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
## 17. Test Coverage Update (v7.x)

### 17.1 New Test Files

| File | Tests | Coverage |
|------|-------|----------|
| `tests/test_proactive_nudging_v70.py` | 18 | Time windows, nudge embeds, status tracking |
| `tests/test_met_calculation.py` | 28 | MET mapping, calorie formula, intensity classification |

### 17.2 Total Test Suite

| Category | Count | Status |
|----------|-------|--------|
| Unit Tests | 133 | ✅ Passing |
| v6.x Tests | 87 | ✅ Passing |
| v7.0 Tests | 18 | ✅ Passing |
| v7.1 Tests | 28 | ✅ Passing |

---

## 18. Capstone Visualization (2026-03-12)

### 18.1 Generated Charts

**Script:** `scripts/generate_capstone_stats.py`

**Output Directory:** `docs/capstone/`

| File | Content |
|------|---------|
| `system_overview.png` | Combined dashboard with all metrics |
| `intensity_distribution.png` | Exercise intensity breakdown (bar + pie) |
| `equipment_analysis.png` | Equipment type distribution |
| `data_completeness.png` | Data quality metrics (progress bars) |

### 18.2 Chart Features

- Professional matplotlib styling
- Health Butler brand colors
- MET threshold annotations
- Bodyweight vs Equipment coverage

---

## 19. Modified Files Summary (v7.x)

| File | Changes |
|------|---------|
| `src/scheduler/__init__.py` | NEW - Module entry point |
| `src/scheduler/time_window_detector.py` | NEW - Pattern detection algorithm |
| `src/data_rag/met_mapping.py` | NEW - MET Science Engine |
| `scripts/enrich_exercise_cache.py` | NEW - Batch enrichment script |
| `scripts/generate_capstone_stats.py` | NEW - Visualization generator |
| `src/discord_bot/bot.py` | Added proactive loops, MET enrichment |
| `src/discord_bot/views.py` | Added ProactiveNudgeView |
| `src/discord_bot/embed_builder.py` | Added visual exercise guide support |
| `src/discord_bot/commands.py` | Added /test_nudge command |
| `src/discord_bot/profile_utils.py` | Added nudge status tracking |
| `src/agents/fitness/fitness_agent.py` | MET-based calorie calculation |
| `tests/test_proactive_nudging_v70.py` | NEW - 18 tests |
| `tests/test_met_calculation.py` | NEW - 28 tests |
| `data/rag/exercise_cache.json` | Enriched with MET values |
...
|---------|------|-------------|
| v7.1 | 2026-03-12 | MET Science Engine, visual exercise guide, capstone viz |
| v7.0 | 2026-03-11 | Proactive nudging, time window detection |
| v6.4 | 2026-03-10 | New user guide, private channels |
...
| v8.5 | 2026-03-01 | M3 baseline (vision, safety, persistence) |
